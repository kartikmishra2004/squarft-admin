...
const Button = ({ children, variant = 'primary', icon: Icon, onClick, className = '', type = "button", disabled = false, ...rest }) => {
    ...
    // Forward remaining native button props (e.g. `title` for a disabled-state
    // tooltip) — needed by the SQF-KYC-023 "missing mandatory documents"
    // Approve-button safeguard in UserList.jsx.
    return (
        <button type={type} onClick={onClick} disabled={disabled} className={`${base} ${variants[variant]} ${className}`} {...rest}>
