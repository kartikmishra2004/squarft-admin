...
const Modal = ({ isOpen, onClose, title, children, size = 'md' }) => {
    ...
    const sizeClass = size === 'xl' ? 'max-w-6xl' : size === 'lg' ? 'max-w-3xl' : 'max-w-lg';
    return (
        <div className="fixed inset-0 z-100 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
            <div className={`bg-white rounded-2xl shadow-2xl w-full ${sizeClass} overflow-hidden animate-in zoom-in-95 duration-200 border border-gray-100 flex flex-col max-h-[90vh]`}>
