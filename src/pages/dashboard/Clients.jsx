...
const ClientProfileView = ({ client, projects, visits, officers, onBack, onUpdateClient, onAddNote, onAddMeeting }) => {
    ...
    const handleAssignSubmit = () => {
        ...
        const pipelineItems = selectedProps.map((projectId) => ({
            projectId,
            status: 'Shown',
            units: [],
            visitedOn: null,
        ...
        const timelineEvent = createTimelineEvent('Properties Assigned', `${selectedProps.length} properties assigned to ${selectedSalesOfficer}`);
        ...
        window.setTimeout(() => {
            ...
            setSelectedProps([]);
            setAssignedOfficer('');
            setActiveProfileTab('Overview & Pipeline');
        }, 900);
    };
                        ...
                        <div className="animate-in fade-in">
                            ...
                            <Card className="mb-8 border-t-4 border-t-[#6F4BFF]">
                                <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                                    ...
                                    <button onClick={handleAssignSubmit} disabled={selectedProps.length === 0 || !selectedSalesOfficer} className="bg-[#6F4BFF] hover:bg-[#5936eb] text-white px-8 py-3 rounded-lg font-bold shadow-md disabled:opacity-50 transition-all flex items-center justify-center gap-2">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {projects.map((project) => {
                                    ...
                                    return (
                                        <Card key={project.id} noPadding className={`relative border-2 transition-all cursor-pointer ${isSelected ? 'border-[#6F4BFF] shadow-md ring-2 ring-[#6F4BFF]/20 bg-purple-50/10' : 'border-gray-200 hover:border-[#6F4BFF]/50'}`}>
                                            <button type="button" className="absolute top-4 right-4 z-20" onClick={() => toggleProperty(project.id)}>
                                                <span className={`w-6 h-6 rounded flex items-center justify-center border-2 transition-colors ${isSelected ? 'bg-[#6F4BFF] border-[#6F4BFF] text-white' : 'bg-white border-gray-300'}`}>
                                                    {isSelected && <Check className="w-4 h-4" />}
                                                </span>
                                            </button>
                                            {isRecommended && <div className="absolute top-4 left-4 z-20"><Badge variant="green">98% Match</Badge></div>}
                                            <button type="button" className="w-full text-left flex gap-4 p-4 pt-12 items-start" onClick={() => toggleProperty(project.id)}>
